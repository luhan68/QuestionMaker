 ...
 */
package algorithms.Equations.Level6_1;
...
public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            correct_answer = (int) Math.pow(first, second);
            
            if (correct_answer < 300) {
                // add question
        ...
        for (int i = 1; i < 6; i++) { //mix in the random answers with the correct answer
            ...
            } else if (inverse_used == false) { //multiply
                results.addElement(first * second);
                inverse_used = true;
            } else if (plusone_used == false) { //add
                results.addElement(first + second);
                plusone_used = true;
            } else if (minusone_used == false) { //reverse
                results.addElement((int)Math.pow(second, first));
                minusone_used = true;
            } else { // random
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second) {
        String results = "";
        String multiplicationLine = "" + first;
        for(int i = 0; i < second - 1; i++){
            multiplicationLine = multiplicationLine + " * " + first;
        }
        if (hint.contains("Algorithm Generated")) {
            results = first + "^" + second + " = " + multiplicationLine;
        } else {
            results = hint;
        }
