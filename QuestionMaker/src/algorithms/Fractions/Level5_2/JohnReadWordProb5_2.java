 ...
 * @author Karl
 */
public class JohnReadWordProb5_2 {
...
public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            second = rn.nextInt(high_threshold - low_threshold) + low_threshold; //denom
            first = rn.nextInt(second * 5) + 1;                                 //num
            String simplifiedOne = fractionHelper.simplifyFractions(first, second);
            fourth = rn.nextInt(high_threshold - low_threshold) + low_threshold;//denom
            third = rn.nextInt(fourth * 5) + 1;                                //num
            String simplifiedTwo = fractionHelper.simplifyFractions(third, fourth);
            
            correct_answer = fractionHelper.Fraction_Math("subtract", first, second, third, fourth);
          //   = fractionHelper.getWithoutWholeNumber();
            
        //    newNum = fractionHelper.getNumSimplified();
        //    newDenom = fractionHelper.getDenomSimplified();
            
            double temp = (first*1.0/second) - (third*1.0/fourth);
        
            if (first > second && third > fourth && first%second != 0
                    && third%fourth != 0 && temp > 0 && second < 6 && fourth < 6) { //the question
                ...
                String question = name + " read for a total of " + simplifiedOne
                        + " hours in two days. On " + dayOne + " " + pronoun + " read "
                        + simplifiedTwo + " hours. How many hours did " + pronoun
                        + " read on " + dayTwo + "?";
        ...
        results.addElement(CreateHint(hint, correct_answer, first, second, third, fourth, dayOne));
    ...
    public String CreateHint(String hint, String correct_answer, int first, int second, int third, int fourth, String dayOne) {
        String results = "";
     ...
     //   String thirdLine = firstNum + "/" + denom + " + " + secondNum + "/" + denom + " =";
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Step 1: Multiply the whole number by the fractions denominator."
                    + " Step 2: Simplify the fractions. Step 3: Subtract the amount read on "
                    + dayOne + " from the total amount read in the two days.";
        } else {
            results = hint;
        }
