...
public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            correct_answer = Math.abs(first);
            
            if (first != 0) { // exclude zero?
                // add question
                String question = "What is the absolute value of " + first + "?";
        ...
        for (int i = 1; i < 6; i++) { //mix in the random answers with the correct answer
            ...
            } else if (inverse_used == false) { //make correct answer negative
                int wrong = correct_answer * -1;
                results.addElement(wrong);
                inverse_used = true;
            } else if (plusone_used == false) { // random
                second = first;
                while(second == first || second == first*-1)
                {
                    second = rn.nextInt(high_threshold - low_threshold)
                        + low_threshold;
                }    
                results.addElement(second);
                plusone_used = true;
            } else if (minusone_used == false) { // switch sign for random
                second = second * -1;
                results.addElement(second);
                minusone_used = true;
            } else { // random
