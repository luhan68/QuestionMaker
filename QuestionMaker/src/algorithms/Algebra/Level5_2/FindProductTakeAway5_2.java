 ...
 */
package algorithms.Algebra.Level5_2;
 ...
 * @author Karl
 */
public class FindProductTakeAway5_2 {
 ...
 public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            first = rn.nextInt(8) + 2;
            second = rn.nextInt(8) + 2;
            third = rn.nextInt(high_threshold - low_threshold) + low_threshold;
            correct_answer = (first * second) - third;
            if (correct_answer >= 0) { // matches criteria
                String question = "Find the product of " + first + " x " + second + ", then take away " + third;
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Multiply " + first + " by " + second + " first, then add " + third;
