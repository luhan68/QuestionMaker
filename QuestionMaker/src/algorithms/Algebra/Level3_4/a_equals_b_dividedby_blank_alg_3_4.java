 ...
 */
package algorithms.Algebra.Level3_4;
 ...
 * @author Karl
 */
public class a_equals_b_dividedby_blank_alg_3_4 {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
           ...
           // first = first + low_threshold;
            ...
            second = second + 1; // no dividing by zero
            
            third = first / second;

            correct_answer = second;
                        
            if (second <= other_threshold && first % second == 0 && third <= other_threshold) {
                String question = third + " = " + first + " / ___";
    ...
    public int Inverse(int first, int second, int third, int correct) {
        int results = 0;
        results = first - third; // using subtraction instead
        if (results < 0) {
            results = 0;
        }
        return results;
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        String results = "";
        String additionLine = "" + third;
        for(int i = 0; i < second - 1; i++){
            additionLine = additionLine + " + " + third;
        }
        if (hint.contains("Algorithm Generated")) {
            results = "How many times was " + second + " added to get to " + third + "? " + additionLine;
      //  results = "hi";
        } else {
            results = hint;
        }
