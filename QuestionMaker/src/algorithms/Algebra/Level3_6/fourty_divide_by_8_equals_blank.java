 ...
 */
package algorithms.Algebra.Level3_6;

import algorithms.Algebra.Level3_4.*;
 ...
 * @author Karl
 */
public class fourty_divide_by_8_equals_blank {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            first = rn.nextInt(other_threshold - 1) + 1;
            first = first + low_threshold + 1;
         ...
         //   second = second + low_threshold;
            ...
            if (third >= low_threshold && third <= high_threshold) {
                String question = third + " / " + first + " = __.";
    ...
    public int Inverse(int first, int second, int third, int correct) {
        int results = 0;
        results = third - second; // using addition instead
        if (results < 0) {
            results = 0;
        }
        return results;
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        String results = "";
        String additionLine = "" + second;
        for(int i = 0; i < first - 1; i++){
            additionLine = additionLine + " + " + second;
        }
        if (hint.contains("Algorithm Generated")) {
            results = "How many times do you add " + first + " to get " + third + "? " + additionLine;
      //  results = "hi";
        } else {
            results = hint;
        }
