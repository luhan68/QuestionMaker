 ...
 */
package algorithms.Algebra.Level3_7;
 ...
 * @author Karl
 */
public class a_times_b_equals_alg_3_7 {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            first = first + 1; // ensures first never equals 1
         ...
         //   second = second + low_threshold;
            ...
            if (third >= low_threshold && third <= high_threshold) {
                String question = first + " times " + second + " = ?";
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
       NumbersToWords num = new NumbersToWords();
        String results = "";
        String multiplicationLine = "" + second;
        for(int i = 0; i < first - 1; i++){
            multiplicationLine = multiplicationLine + " + " + second;
        }
        if (hint.contains("Algorithm Generated")) {
            results = "Add " + first + " " + num.conversion(second) + " times. " + multiplicationLine ;
        } else {
            results = hint;
        }
