    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            if (correct_answer >= low_threshold && correct_answer <= high_threshold) { // matches criteria
                String question = Name + " Has $" + first + ". "+ capital.Capitalize(names.GenderFinder(Name))+ " Mom gives " + names.GenderFinder(Name)+" $" + second 
                        + ". " + capital.Capitalize(names.GenderFinder(Name)) + " Dad gives " + names.GenderFinder(Name) + " $" + third + 
                      " more. How many dollars does " + Name + " have now?";
  ...
  public String HintImg() {
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third, String item) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Use number line. Start at " + first + "~ add " + second + "~ then add " + third + "."  ;
