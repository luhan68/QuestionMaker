 ...
 * @author Karl
 */
public class AreaCircleGivenDiameter {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            double radius = first/2.0;
            correct_answer = second * Math.pow(radius, 2);
            correct_answer = fractionHelper.round(correct_answer, 3); // three decimals
            
            if (true) { // matches criteria
                // add question
                String question = "Find the area of a circle with diameter " + first + " m";
