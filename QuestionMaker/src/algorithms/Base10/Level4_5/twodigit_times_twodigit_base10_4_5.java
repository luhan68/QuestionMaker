 ...
 * @author Karl
 */
public class twodigit_times_twodigit_base10_4_5 {
...
public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        String results = "";
        String equation = "(" + (first/10*10) + "x" + (second/10*10) + ") + (" 
                + (first/10*10) + "x" + (second%10) + ") + (" + (first%10) + "x"
                + (second/10*10) + ") + (" + (first%10) + "x" + (second%10) + ") = ?";        
