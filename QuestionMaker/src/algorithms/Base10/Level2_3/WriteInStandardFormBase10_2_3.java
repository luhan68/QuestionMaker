 ...
 * @author Karl
 */
public class WriteInStandardFormBase10_2_3 {
...
public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            first = helper.ConvertNumberToWords(correct_answer);
            
            if (correct_answer <= high_threshold && correct_answer >= low_threshold) {
                String question = "What number is " + first + " in standard form?";
    ...
    public String CreateHint(String hint, int correct_answer, String first, int second, int third) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Use place chart. A example of standard for is 734.";
        } else {
            results = hint;
        }
