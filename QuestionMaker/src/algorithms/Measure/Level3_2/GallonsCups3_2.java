 ...
 */
package algorithms.Measure.Level3_2;
...
public Vector generate(int low_threshold, int high_threshold, String hint, String level, String subjectype) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            correct_answer = first * 4 * 4;
            
            if (true) { //the question
                String question = first+" gallons = __ cups";
        ...
        for (int i = 1; i < 6; i++) { //mix in the random answers with the correct answer
            if (i == Random_correct_letter) { //this is the randomly selected letter
                ...
                while(answer == correct_answer || results.contains(answer)){
                    answer = rn.nextInt(high_threshold + 10) + 1;
                }
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "1 gallon = 4 quarts  &  1 quart = 4 cups";
        } else {
            results = hint;
        }
