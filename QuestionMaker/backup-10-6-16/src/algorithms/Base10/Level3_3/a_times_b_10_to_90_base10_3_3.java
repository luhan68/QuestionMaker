 ...
 */
package algorithms.Base10.Level3_3;
 ...
 * @author Karl
 */
public class a_times_b_10_to_90_base10_3_3 {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level) {
        ...
        int first = 0;
        int first_threshold = 9;
        int second = 0; // the range for second will be the low_threshold and high_threshold params
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            first = rn.nextInt(first_threshold) + 1;
            // make second a multiple of 10
        ...
        for (int i = 1; i < 6; i++) { //mix in the random answers with the correct answer
            ...
            if (i == Random_correct_letter) { //this is the randomly selected letter
                ...
                int randomanswer = rn.nextInt(high_threshold + 800) + 1;            
    ...
    public int Inverse(int first, int second, int third, int correct) {
        ...
        if (second > first) {
            ...
            results = second + first; // using addition instead
            if (results < 0) {
                results = 0;
            }
        }
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Think of your basic facts: " + first + " x " + (second / 10) + " = ";
