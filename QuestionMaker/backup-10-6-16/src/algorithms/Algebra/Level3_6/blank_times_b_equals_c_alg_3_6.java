 ...
 * @author Karl
 */
public class blank_times_b_equals_c_alg_3_6 {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
         ...
         //   second = second + low_threshold;
            ...
            if (third >= low_threshold && third <= high_threshold) {
                String question = "One answer for both blanks:~" + third + " / " + second + " = ___~"
                        + "___ x " + second + " = " + third;
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        String results = "";
        int soFar = second;
        String multiplicationLine = "" + soFar;
        for(int i = 0; i < first - 1; i++){
            soFar = soFar + second;
            multiplicationLine = multiplicationLine + ", " + soFar;
        }
        if (hint.contains("Algorithm Generated")) {
            results = multiplicationLine;
        } else {
            results = hint;
        }
