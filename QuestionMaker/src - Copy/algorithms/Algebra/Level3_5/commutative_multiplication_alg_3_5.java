 ...
 */
package algorithms.Algebra.Level3_5;
 ...
 * @author Karl
 */
public class commutative_multiplication_alg_3_5 {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            first = rn.nextInt(other_threshold) + 1;
           // first = first + low_threshold;
            
            second = (rn.nextInt(other_threshold) + 1);
         //   second = second + low_threshold;
            ...
            if (third >= low_threshold && third <= high_threshold) {
                String question = "If " + first + " x " + second + " = " + third
                        + ", then " + second + " x ___ = " + third;
    ...
    public int Inverse(int first, int second, int third, int correct) {
        int results = 0;
        results = third - second; // using subtraction instead
        if (results < 0) {
            results = 0;
        }
        return results;
    ...
    public String CreateHint(String hint, int correct_answer, int first, int second, int third) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "The commutive property says: you are allowed to move the numbers around so that a x b = b x a"
                    + " This only works with all multiplication or all addition";
        } else {
            results = hint;
        }
