 ...
 * @author Karl
 */
public class InOutMultiply_algebra5_3 {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            third = rn.nextInt(8) + 2;
            second = first * third;
            
            correct_answer = "a x " + third + " = b";
        ...
        for (int i = 1; i < 6; i++) { //mix in the random answers with the correct answer
            ...
            if (i == Random_correct_letter) { //this is the randomly selected letter
                ...
                results.addElement("a x " + (third+1) + " = b");
                plusone_used = true;
            } else if (minusone_used == false) { // - 1
                results.addElement("a x " + (third-1) + " = b");
                minusone_used = true;
            } else { // random question within the threshold
                int random = rn.nextInt(high_threshold) + 2;
                if (random == third) {
                    random = random + 5;
                }
