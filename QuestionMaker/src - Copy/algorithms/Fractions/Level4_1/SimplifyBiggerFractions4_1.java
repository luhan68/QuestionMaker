 ...
 * @author Karl
 */
public class SimplifyBiggerFractions4_1 {
    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level) {
        ...
        while (found == false) { // try to produce a formula that is within the bounds
            ...
            if (first < second && !correct_answer.equals(third) && newDenom <= 10) { //the question
                String question = "Reduce the fraction as much as possible: "
                        + third + " =";
        ...
        for (int i = 1; i < 6; i++) { //mix in the random answers with the correct answer
            if (i == Random_correct_letter) { //this is the randomly selected letter
                ...
                String fraction = newNum + "/" + (newDenom + 5);
                results.addElement(fraction);
            }
        }
    ...
    public String CreateHint(String hint, String correct_answer, int first, int second, String third) {
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Divide the numerator and the denominator by the same number.";
        } else {
            results = hint;
        }
