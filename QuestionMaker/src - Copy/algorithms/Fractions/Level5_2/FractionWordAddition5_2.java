    ...
    public Vector generate(int low_threshold, int high_threshold, String hint, String level) {
        ...
        //    newDenom = fractionHelper.getDenomSimplified();
            ...
            if (first > second && third > fourth && first%second != 0 && third%fourth != 0) { //the question
                String question = name + " ran " + simplifiedOne + " miles on Monday and "
                + simplifiedTwo + " miles on Tuesday. How many total miles did " + name + " run both days?";
    ...
    public String CreateHint(String hint, String correct_answer, int first, int second, int third, int fourth) {
     ...
     //   String thirdLine = firstNum + "/" + denom + " + " + secondNum + "/" + denom + " =";
        ...
        if (hint.contains("Algorithm Generated")) {
            results = "Add the whole nunbers together, then add the fractions together, "
                    + "then combine the whole number and the fraction.";
        } else {
            results = hint;
        }
